...
| **F0.2** | **Build distribuível e padrão visual** — Sandbox fora da build; configs para `Data/Config` (o `Resources.Load` do `DebugConfig` retorna null hoje, e os 167 call sites de log ficam sem filtro); CanvasScaler consistente — `MainMenu`/`Loading` usam `ConstantPixelSize` 800×600 e **não escalam**, então é re-layout, não toggle; `Application.Quit` e tela de opções, ambos ausentes. **Inclui adotar o `UITheme`**, que existe e é bom mas quase não é usado (`ThemedGraphic` em 3 arquivos, `ThemedButton` em 1, `ThemedText` nem existe) — é aqui que o padrão visual herdado por ~10 telas do F4/F5 é definido. Ver `Docs/plans/F0.2-build-distribuivel.md` | 4-6 sem |
